...
//  HARE KRISHNA
//  SreeKajol_World (◉_◉)
//Consistency is the key ... Practice over yourself!
#include<bits/stdc++.h>
using namespace std;
...
#define numSize(n) floor(log10(n)) + 1; //O(1)
   ...
   auto SreeKajol = [&](int Case) /* ->vector<int> */ {  
    ...
    while(cin>>n){
        ...
        while(idx<prime.size() && prime[idx]<=n){
            int need= n-prime[idx];
            auto lb= lower_bound(prime.begin(), prime.end(), need);
            if(lb!=prime.end() && *lb==need){
                cout<<n<<" = "<<prime[idx]<<" + "<<need<<"\n";
                flag=false;
                break;
            }
            else if(lb!=prime.end()){
                idx++;
            }
        }
        if(flag){
